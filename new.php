...
// ===== NEW: TABLE FOR WHATSAPP MESSAGES =====
...
// ========== NEW: SEND WHATSAPP MESSAGE FUNCTION ==========
function sendWhatsAppMessage($to, $message, $conn) {
    $to = $conn->real_escape_string($to);
    $message = $conn->real_escape_string($message);
    $sql = "INSERT INTO whatsapp_messages (phone, message) VALUES ('$to', '$message')";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (isset($_POST['cart_checkout']) && $_POST['cart_checkout'] == 1) {
                ...
                if ($has_pending) $message = "You have a pending request for one or more courses.";
                    ...
                    if (createPaymentRequest($conn, $uid, $cart_items, $full, $email, $txn, $amt, $payment_method, $payment_number)) {
                        ...
                        if (!empty($payment_number)) {
                            $videoCount = 8;
                            $userMsg = "KUSOO DHAWOOW SALAAM ONLINE\n\nWaxaad dalbatay koorsooyin: " . implode(', ', $courseNames) . "\nLacag: $$amt\nWaxaana jira $videoCount videos per course.\nFariintaada waa la soo dhoweeyay, waxaa lagu soo jawaabi doonaa 24 saac gudahood.\nMahadsanid!";
                            sendWhatsAppMessage($payment_number, $userMsg, $conn);
                            
                            $adminMsg = "Macmiil cusub: $full\nWuxuu dalbaday: " . implode(', ', $courseNames) . "\nLacag: $$amt\nTransaction: $txn\nMethod: $payment_method\nLambar: $payment_number";
                            sendWhatsAppMessage('0634621822', $adminMsg, $conn);
                            $whatsapp_status = "WhatsApp messages sent to $payment_number and admin.";
                        }
            ...
            if (hasPendingRequest($conn, $uid, $cid)) $message = "You already have a pending request for this course.";
                ...
                if (createPaymentRequest($conn, $uid, $cid, $full, $email, $txn, $amt, $payment_method, $payment_number)) {
                    ...
                    // ===== NEW: SEND WHATSAPP MESSAGES =====
                    if (!empty($payment_number)) {
                        $courseName = isset($course_list[$cid]) ? $course_list[$cid]['name'] : 'Course';
                        $videoCount = 8;
                        $userMsg = "KUSOO DHAWOOW SALAAM ONLINE\n\nWaxaad dalbatay kooris: $courseName\nLacag: $$amt\nWaxaana jira $videoCount videos.\nFariintaada waa la soo dhoweeyay, waxaa lagu soo jawaabi doonaa 24 saac gudahood.\nMahadsanid!";
                        sendWhatsAppMessage($payment_number, $userMsg, $conn);
                        
                        $adminMsg = "Macmiil cusub: $full\nWuxuu dalbaday: $courseName\nLacag: $$amt\nTransaction: $txn\nMethod: $payment_method\nLambar: $payment_number";
                        sendWhatsAppMessage('0634621822', $adminMsg, $conn);
                        $whatsapp_status = "WhatsApp messages sent to $payment_number and admin.";
                    }
                } else $message = "Error sending request.";
    ...
    // ... rest of POST handlers remain unchanged ...
...
<head>
    ...
    <style>
        ...
        .whatsapp-notification .material-symbols-outlined {
            font-size: 2rem;
        }
        .whatsapp-logs-table {
            background: white;
...
<main class="mt-16">
<?php
// ========== PAGE RENDERING (unchanged except for admin WhatsApp logs) ==========
...
elseif ($page == 'admin' && isAdmin($user)):
    // Fetch WhatsApp logs for admin
    $whatsappLogs = $conn->query("SELECT * FROM whatsapp_messages ORDER BY sent_at DESC LIMIT 50");
?>
    <div class="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop py-12">
        ...
        <!-- New: WhatsApp Logs Table for Admin -->
        <div class="mt-12 bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
            <h2 class="text-xl font-bold mb-4 flex items-center gap-2">
                <span class="material-symbols-outlined text-green-600">whatsapp</span> WhatsApp Messages Log (Last 50)
            </h2>
            <div class="overflow-x-auto">
                <table class="min-w-full border border-gray-200 rounded-lg">
                    <thead class="bg-gray-100">
                        <tr>
                            <th class="px-4 py-2 text-left text-xs font-medium text-gray-600 uppercase">Phone</th>
                            <th class="px-4 py-2 text-left text-xs font-medium text-gray-600 uppercase">Message</th>
                            <th class="px-4 py-2 text-left text-xs font-medium text-gray-600 uppercase">Sent At</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200">
                        <?php if($whatsappLogs->num_rows > 0): ?>
                            <?php while($log = $whatsappLogs->fetch_assoc()): ?>
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 py-2 text-sm font-mono"><?php echo htmlspecialchars($log['phone']); ?></td>
                                    <td class="px-4 py-2 text-sm whitespace-pre-wrap"><?php echo nl2br(htmlspecialchars($log['message'])); ?></td>
                                    <td class="px-4 py-2 text-sm"><?php echo date('M d, H:i', strtotime($log['sent_at'])); ?></td>
                                </tr>
                            <?php endwhile; ?>
                        <?php else: ?>
                            <tr><td colspan="3" class="text-center py-4 text-gray-500">No WhatsApp messages sent yet.</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
...
<script>
    ...
    setTimeout(function(){
        var div = document.createElement('div');
        div.className = 'whatsapp-notification fixed bottom-4 left-4 z-50';
        div.innerHTML = '<span class="material-symbols-outlined">whatsapp</span> ' + '<?php echo addslashes($whatsapp_status); ?>' + '<button onclick="this.parentElement.remove()" class="ml-4 text-white/70 hover:text-white">&times;</button>';
        document.body.appendChild(div);
